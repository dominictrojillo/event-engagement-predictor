...
if not PIPE.exists():
    st.error("Model missing. Run: python3 src/train_model.py first.")
    st.stop()
...
with col1:
    ...
    likes = st.number_input("Instagram Likes", value=50)
    comments = st.number_input("Instagram Comments", value=5)
...
if st.button("Predict"):
    ...
    if show_shap:
        ...
        shap_values = explainer.shap_values(pipeline["pre"].transform(input_df))
